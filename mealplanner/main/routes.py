...
def find_by_ingredients():
    ...
    for meal in candidates:
        ...
        if len(missing) == 0:
            exact_matches.append(meal)
        elif len(missing) <= 2:
            near_matches.append({'meal': meal, 'missing': missing})

    exact_matches.sort(key=lambda m: m.name.lower())
    near_matches.sort(key=lambda x: (len(x['missing']), x['meal'].name.lower()))

    return render_template('main/find_by_ingredients.html', query=query, exact_matches=exact_matches, near_matches=near_matches, used_storage=used_storage)